...
void ch10__sparse_matrix_device(double *h_input, double *h_output, const int length, kernel_config_t config){
...
void ch10__sparse_matrix_host(double *input, double *output, const int length){
	HOST_TIC(0);
	HOST_TOC(0)
}
...
void ch10__sparse_matrix(env_e env, kernel_config_t config){
	...
	if(env == Host){
		ch10__sparse_matrix_host(input, output, CH9__ARRAY_LENGTH);
	}else{
		ch10__sparse_matrix_device(input, output, CH9__ARRAY_LENGTH, config);
	}
